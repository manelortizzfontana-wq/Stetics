...
const WorkoutTracker = {
    ...
    // Iniciar entrenamiento desde una rutina o en blanco (con soporte de días específicos)
    startWorkout(routineOrNull = null, selectedDayOrNull = null) {
        ...
        const now = new Date();
        if (routineOrNull) {
            let sessionTitle = routineOrNull.name.replace(/^[^\w\s]+/, '').trim();
            let targetExercises = [];

            if (selectedDayOrNull) {
                sessionTitle = `${sessionTitle} • ${selectedDayOrNull.name.replace(/^[^\w\s]+/, '').trim()}`;
                targetExercises = selectedDayOrNull.exercises || [];
            } else if (routineOrNull.days && routineOrNull.days.length > 0) {
                const firstDay = routineOrNull.days[0];
                sessionTitle = `${sessionTitle} • ${firstDay.name.replace(/^[^\w\s]+/, '').trim()}`;
                targetExercises = firstDay.exercises || [];
            } else {
                targetExercises = routineOrNull.exercises || [];
            }
            ...
            this.activeSession = {
                ...
                durationSeconds: 0,
                exercises: targetExercises.map(item => {
                    ...
                    return {
                        ...
                        name: dbEx ? dbEx.name : (item.name || 'Ejercicio'),
